...
F4=5 + 17 + 5×17 = 107*/
    ...
    // breaking f(n) = f(n-1) + f(n-2) +f(n-1)f(n-2) ( as it looks alot like fibonaci series recur relation we try to make it into that)
    // into more simplified form 
...
//     G(n) = G(n-1)G(n-2)
    //let G(0)=a,G(1)=b
    //G(2)=ab,G(3)=a(b^2)
    // G(n) = power(a,fib(n-1) x power(b,fib(n)
    // but fib(n-1) can be hugeeeee 
    // so we will use fermets theorem and we get 

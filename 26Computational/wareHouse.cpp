...
using namespace std;
//my logic 
// it is passing two out of three hidden testcases
//possible breaking points when
int main()
{
    int n;
    cin >> n;
    ...
    //acc to me x or east direction position will not matter at all
    //we just have to focus on  maximu distance possible  btw two posts in north ; think of it as .......
    //
    
    int ans=y[0];
    sort(y.begin(),y.end());
    if(y.size() == 1)//for e.g 
        cout<<max(y[0],200-y[0]) -1<<endl;// y is 90 then ans is 109 ,y is 
    else{
    for(int i=1;i<n;i++)
    {
        if(y[i] - y[i-1] > ans)
